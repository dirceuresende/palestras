...
SELECT * FROM CLR.dbo.fncEvent_Viewer_Listar('dirceu-notebook', 'Application', '2019-09-30 19:00', '2019-10-01')
...
-- WEB
SELECT * FROM CLR.dbo.fncFeriados(2019, 'BE', 'Belém')
...
EXEC CLR.dbo.stpEnvia_Mensagem_Slack
    @Ds_Mensagem = N'Teste de envio pelo Slack', -- nvarchar(4000)
    @Ds_Canal = N'#ti' -- nvarchar(256)




EXEC CLR.dbo.stpEnvia_Mensagem_Whatspp
    @Destino = '27988543306', -- varchar(20)
    @Mensagem = 'Teste de API do Whatsapp' -- varchar(max)
...
PRINT CLR.dbo.fncDescriptografa_Objeto('dirceu-notebook\sql2017', 'eventos', 'dbo', 'stpProcedure_Criptografada')
...
EXEC CLR.dbo.stpWs_Requisicao
    @Ds_Url = N'https://rastrojs.herokuapp.com/track/PX526034827BR/json', -- nvarchar(max)
...
IF (OBJECT_ID('eventos.dbo.Teste') IS NOT NULL) DROP TABLE eventos.dbo.Teste
EXEC CLR.dbo.stpImporta_Txt_Tamanho_Fixo
    @caminho = N'C:\Temporario\Tamanho Fixo.txt', -- nvarchar(max)
    @Titulos = N'Nome,Site,Idade', -- nvarchar(max)
    @Tamanhos = N'17,23,3', -- nvarchar(max)
    @Ds_Encoding = N'utf-8', -- nvarchar(max)
    @Ds_Tabela_Destino = N'eventos.dbo.Teste', -- nvarchar(max)
...
SELECT *
FROM CLR.dbo.fncRegEdit_Listar('dirceu-notebook', 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL14.SQL2017\Setup')
